...
class CarLights {
  ...
  init() {
    ...
    const distUniforms = typeof this.options.distortion === 'object' ? this.options.distortion.uniforms : {};
    const material = new THREE.ShaderMaterial({
      fragmentShader: carLightsFragment,
      vertexShader: carLightsVertex,
      transparent: true,
      uniforms: Object.assign({ uTime: { value: 0 }, uTravelLength: { value: options.length }, uFade: { value: this.fade } }, this.webgl.fogUniforms, distUniforms)
    });
...
class LightsSticks {
  ...
  init() {
    ...
    const distUniforms = typeof options.distortion === 'object' ? options.distortion.uniforms : {};
    const material = new THREE.ShaderMaterial({
      fragmentShader: sideSticksFragment,
      vertexShader: sideSticksVertex,
      side: THREE.DoubleSide,
      uniforms: Object.assign({ uTravelLength: { value: options.length }, uTime: { value: 0 } }, this.webgl.fogUniforms, distUniforms)
    });
    material.onBeforeCompile = shader => {
      shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', typeof this.options.distortion === 'object' ? this.options.distortion.getDistortion : '');
    };
...
class Road {
  ...
  createPlane(side: number, _width: number, isRoad: boolean) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      side: THREE.DoubleSide,
      uniforms: Object.assign(uniforms, this.webgl.fogUniforms, distUniforms)
    });
    material.onBeforeCompile = shader => {
      shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', typeof this.options.distortion === 'object' ? this.options.distortion.getDistortion : '');
    };
...
class App {
  ...
  tick() {
    ...
    if (this.hasValidSize) { const delta = this.clock.getDelta(); this.composer.render(delta); this.update(delta); }
